...
export class ContactService {
    ...
    getByCustomer(idCustomer: string, expandables: string[]): Observable<TotvsResponse<IContact>> {
        let lstExpandables = this.getExpandables(expandables);
        if (lstExpandables !== '') { lstExpandables = `&${lstExpandables}`; }

        return this.http.get<TotvsResponse<IContact>>(`${this.apiBaseUrl}?customer=${idCustomer}${lstExpandables}`, this.headers);
    }
